...
class PessoasDb(object):
    ...
    def remover_registros(self):
        tabela = "CIDADES"
        sql_delete = "DELETE FROM " + tabela
        
        try:
            self.db.cursor.execute(sql_delete)
            self.db.commit_db()
        except:
            print("Nao foi possivel remover registros tabela %s" % tabela)
            return False
    ...
    def inserir_de_csv(self, file_name='csv/cidades.csv'):
        try:
            ...
            for t in c:
                ...
                INSERT INTO cidades (id, cidade, uf)
                VALUES (?,?,?)
                """, (indice, t[0], t[1]))
            # gravando no bd
            self.db.commit_db()
...
class ClientesDb(object):
    ...
    def remover_registros(self):
        try:
            ...
            """)
            self.db.commit_db()
            print("Registros removidos da base de dados!")
            
        except:
            print("Aviso: Nao foi possivel remover registros da base de dados!")
            return False
    ...
    def imprimir_cliente(self, id):
        ...
        if cliente == None:
            print('Não existe cliente com o id informado {%d}' % id)
        else:
    ...
    def deletar(self, id=50):
        try:
            ...
            if c:
                ...
                print('Não existe cliente com o código informado {%d}' % id)
        except e:
            raise e
...
if __name__ == '__main__':
    # ClientesDb
    print("===========================================================")
    print("Iniciando processamento registros classe ClientesDb")
    print("===========================================================")
    c = ClientesDb()
    c.criar_schema()
    c.remover_registros() # Sempre remove registros para realizar novos testes
    c.inserir_um_registro()
    c.inserir_com_lista()
    c.inserir_de_arquivo()
    c.inserir_de_csv()
    #c.inserir_com_parametros()
    c.inserir_randomico(7)
    c.imprimir_todos_clientes()
    c.imprimir_cliente(gen_int())
    c.contar_cliente()
    c.contar_cliente_por_idade()
    c.localizar_cliente_por_idade()
    c.localizar_cliente_por_uf()
    ...
    # PessoaDb
    print("===========================================================")
    print("Iniciando processamento registros classe PessoasDb")
    print("===========================================================")
    p = PessoasDb()
    p.criar_schema()
    p.remover_registros() # Sempre remove registros para realizar novos testes
    p.inserir_de_csv()
    p.gen_cidade()
